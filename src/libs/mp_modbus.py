...
class MpModbus:
    ...
    def showCurrentState(self):
        print("--- Current State ---")
        print("Set Point L1 : %d" % (self._readRegister(UNIT_MPLUS_VEBUS, 37, INT16)))
        print("Set Point L2 : %d" % (self._readRegister(UNIT_MPLUS_VEBUS, 40, INT16)))
        print("Set Point L3 : %d" % (self._readRegister(UNIT_MPLUS_VEBUS, 41, INT16)))

        #   print("Disable Charge : %d" %
        #         (self._readRegister(UNIT_MULTI, 38 UINT16)))
        print(
            "Disable Feed-in: %d" % (self._readRegister(UNIT_MPLUS_VEBUS, 39, UINT16))
        )
        print(
            "Disable overvoltage Feed-in : %d"
            % (self._readRegister(UNIT_MPLUS_VEBUS, 65, UINT16))
        )

        print(
            "Maximum Feed-in power due to overvoltage L1 : %d"
            % (self._readRegister(UNIT_MPLUS_VEBUS, 66, UINT16))
        )
        print(
            "Maximum Feed-in power due to overvoltage L2 : %d"
            % (self._readRegister(UNIT_MPLUS_VEBUS, 67, UINT16))
        )
        print(
            "Maximum Feed-in power due to overvoltage L3 : %d"
    ...
    def getDcPvPower(self):
        try:
            ...
            # ---- Battery ---
            batteryVoltage = (
                int(self._readRegister(UNIT_SOLAR_CHARGER, 771, UINT16)) / 100
            )
            self.log.debug("Battery Voltage: %d V" % batteryVoltage)
            batteryCurrent = (
                int(self._readRegister(UNIT_SOLAR_CHARGER, 772, INT16)) / 10
            )
    ...
    def _writeControlledPower(self, power, disableCharging=0, disableInverter=0):
        ...
        self.log.debug(
            "Multiplus->_writeControlledPower(%d,%d,%d)"
